...
CHUNK_LENGTH = magick_number = 4

# проверка длины блока
assert not CHUNK_LENGTH % magick_number, 'Длина блока должна быть кратна magick_number'
...
def chars_to_bin(chars):
    ...
    """
    assert not len(chars) * magick_number % CHUNK_LENGTH, 'Длина кодируемых данных должна быть кратна длине блока кодирования'
    return ''.join([bin(ord(c))[2:].zfill(magick_number) for c in chars])
...
def get_check_bits_data(value_bin):
    ...
    for index, value in enumerate(value_bin, 1):
        if int(value):
            bin_char_list = list(bin(index)[2:].zfill(magick_number))
...
def decode(encoded, fix_errors=True):
    ...
    for clean_chunk in clean_chunk_list:
        for clean_char in [clean_chunk[i:i + magick_number] for i in range(len(clean_chunk)) if not i % magick_number]:
